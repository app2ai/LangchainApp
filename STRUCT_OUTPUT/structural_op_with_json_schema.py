...
structuralModel = ChatOpenAI().with_structured_output(review_json_schema, method="function_calling") # or ChatOpenAI(model="gpt-3.5-turbo", response_format=UserInfo)
...
print(response['name'])  # Accessing the name field from the response "response = Vishal Rana"

#code(response) 
# {'key_themes': ['Large Display', 'Strong Battery Life', 'Clean Android Experience', 'Budget-Friendly'], 
# 'summary': 'The Motorola Moto G67 isabudget-friendly smartphone with a large display, strong battery life,
#  and clean Android experience. It is suitable for everyday users who prioritize reliability and battery life 
# over raw power. However, it may not be ideal for heavy gaming or multitasking.', 'sentiment': 'positive', 
# 'pros': ['Excellent Battery Life', 'Big & Smooth Display', 'Good Daylight Camera', 'Clean Android Experience', 
# 'Durable Design'], 'cons': ['Average Performance', 'Low RAM', 'Weak Low-Light Camera', 'Average Speakers', 
# 'Limited Software Updates'], 'name': 'Vishal Rana'}